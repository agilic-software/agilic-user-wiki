...
The most basic layer. Anyone added to your organization can sign in and use Agilic, scoped to whatever Work Packages and Teams they're added to. Managed from Org Admin → People.
...
![Org People List](media/02c-001.png)
...
> **See also:** For creating an organization for the first time, see [Setting Up Your Organization](/section-2-setting-up-your-org/setting-up-your-organization.md) (Section 2).
...
![Org Role List](media/02c-002.png)

**Teams**

After adding someone to your organization, you can then add them to a Team. People without a Team can still be added to Work Packages and everything else — a Team isn't a prerequisite for anything.

Being on a Team gives you access to that Team's dedicated views.

![Team membership](media/02c-003.png)
...
![Billing Admin flag](media/02c-004.png)
...
Having access to a WP Timesheet is also how your personal Timesheet and your Team Timesheet get populated.
...
![Owner/Driver permissions](media/02c-005.png)
...
> **See also:** For adding and managing Client Contacts, see Managing Client Companies & Contacts (Section 10). For granting a specific Work Package's Client Portal Access, see Work Package Client Portal Management (Section 6).

## Quick Reference

| Access Layer | Controlled Where | Governs |
|---|---|---|
| Organization Membership | Org Admin → Org People List | Whether someone can sign in at all |
| Role | Org Admin → Org Role List | Job title/skillset — not a permission |
| Standard Permission Types (WP Creator, Team Creator, Org Reporting, Admin) | Org Admin → Org People List | Specific org-level capabilities |
| Billing Admin | Org Admin → Billing Admin (1 user only, must already be Org Admin) | Access to Billing / AI Usage tabs |
| WP Roster Membership | Inside a Work Package → Roster | Edit/Assign/Timesheet access within that WP |
| Owner/Driver | Set on the Work Package | Full field-level control of that WP |
| Secure WP / Secure Item (Planned, not yet implemented) | Set by Owner/Driver or Org Admin | Restricts visibility to specific people |
| Client Portal Access | Customer Management / WP Client Portal Mgmt | External client visibility |
